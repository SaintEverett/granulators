/*
    name: 'thinkpad_recv_ambigrani.ck'
...
class GPS
{
    ...
    float coefficients_targets[9]; // x,y,z,w,r,s,t,u,v
    float angles[2];
    float revmix;
...
/* Enable if you want to prevent making more than x amount of GPS(s)
    else if( howMany > x )
    {
        <<< "too many!!" >>>;
        me.exit();
...
int keyArray[4]; // stores the current granulator you want to edit
...
for( int i; i < buffs.size(); i++ ) // turn off buffer interpolation (fun sounding)
{
    buffs[i].interp(2);
}
