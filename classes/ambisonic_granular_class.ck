public class AmbiGranulator extends Chugraph
{
    SndBuf buffer;
    WinFuncEnv env[2];
    ...
    0.0 => float azimuth; // azimuth
    0.0 => float zenith; // zenith
    0.0 => float aziVelocity; // azimuthal velocity
    0.0 => float zeniVelocity; // zenith velocity

    2 => int dimension;

    Event newAziZen;

    fun void AmbiGranulator(string file)
    {
        file => filename;
    ...
    fun void play()
    {
        ...
        spork ~ ramp_gain();
        spork ~ ramp_pitch();
        spork ~ azimuthVelocity();
        if(dimension - 2) spork ~ zenithVelocity();
        spork ~ grain();
    }
    ...
    // azimuth velocity
    fun void azimuthVelocity()
    {
        float deltaAzi;
        while( true )
        {
            aziVelocity * (now/second) => deltaAzi;
            azimuth + deltaAzi => azimuth;
            if( azimuth > 360.0 ) azimuth - 360.0 => azimuth;
    ...
    // zenith velocity
    fun void zenithVelocity()
    {
        float deltaZen;
        while( true )
        {
            zeniVelocity * (now/second) => deltaZen;
    ...
    fun void Velocities()
    {
        cherr <= IO.newline() <= "Azimuth Velocity: " <= aziVelocity <= " Zenith Velocity: " <= zeniVelocity <= IO.newline();
    }
}
